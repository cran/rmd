...
#' @import shiny
rmdAddin <- function() {
  resourcePath <- system.file("gadgets", "rmd", package = "rmd")
  system.file("lib", "sweetalert-1.0.1", package = "rmd")
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(input$rowclick, {
      ...
      if (pkg == "rmd") {
        shinyjs::js$swal(title = "", text = "Cannot uninstall rmd, it is required for the current app to work",
                         type = "info")
        return()
      }
    ...
    output$addinstable <- DT::renderDataTable({
      DT::datatable(
        ...
        options = list(
          dom = "iftlp",
          language = list(
            zeroRecords = "No package found",
            info = "_TOTAL_ packages found",
            infoFiltered = "",
            infoPostFix = " (click any row to install/uninstall the package)",
  ...
  viewer <- dialogViewer("Discover and install useful R markdown packages", width = 1200, height = 900)
